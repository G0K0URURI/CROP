...
class CROPTrainer(BasicModelBasedOfflineRLTrainer):
    def __init__(self, state_dim, action_dim, env_mlp_layer_number, env_mlp_hidden_size,
        ...
        self.env_state_optimizers = [optim.Adam([{'params': self.env_models[i].state_predictor.parameters(), 'weight_decay':0.000075}],
        ...
        self.env_reward_optimizers = [optim.Adam([{'params': self.env_models[i].reward_predictor.parameters()}],
                                                 lr=env_lr, weight_decay=0.000075)
                                      for i in range(env_nums)]
        # vectorize model ensembling to fast roll out(https://pytorch.org/functorch/1.13/notebooks/ensembling.html)
        self.env_ensemble_function = None
        self.env_ensemble_params = None
        self.env_ensemble_buffers = None
    ...
    def env_state_loss(self, state, action, next_state, env_index=0):
        next_state_mean, next_state_std = self.env_models[env_index].pre_next_state_mean_and_std(state, action)
    ...
    def env_reward_loss(self, state, action, reward, env_index=0, random_action_num=1):
        ...
        reward_advantage = torch.zeros_like(action)
    ...
    def env_state_train_step(self, state, action, next_state, env_index):
        next_state_mean_loss, next_state_var_loss = self.env_state_loss(state, action, next_state, env_index)

        loss = next_state_mean_loss.mean() + next_state_var_loss.mean()
        self.env_state_optimizers[env_index].zero_grad()
        loss.backward()
        self.env_state_optimizers[env_index].step()
    ...
    def env_state_valid_step(self, state, action, next_state, env_index):
        #
        with torch.no_grad():
            next_state_mean_loss, next_state_var_loss = self.env_state_loss(state, action, next_state, env_index)
            ...
            loss = next_state_mean_loss.mean() + next_state_var_loss.mean()
            logger = {
                'next_state_mean_loss_valid': next_state_mean_loss.mean().item(),
                'next_state_var_loss_valid': next_state_var_loss.mean().item(),
                'state_loss_valid': loss.item()
            }
        return logger
    ...
    def env_state_train(self, batch_size, epoch, save_dir, valid_size=5000, max_epochs_since_update=5, env_index=0):
        ...
        for i in range(int(epoch) + 1):
            ...
            for batch_num in range(int(np.ceil(train_size / batch_size))):
                ...
                logger = self.env_state_train_step(state, action, next_state, env_index)
            if i % 1 == 0:
                with torch.no_grad():
                    next_state_mean, _ = self.env_models[env_index].pre_next_state_mean_and_std(state, action)
                    logger['state_mse_loss'] = torch.square(next_state_mean - next_state).mean().item()

                    state, action, reward, next_state, done = valid_buffer.sample(len(valid_buffer))
                ...
                with open(os.path.join(save_dir, 'model_state_log.csv'), 'a') as f:
                    f.write(info + '\n')
                self.save_env_part(os.path.join(save_dir, 'model_state_epoch%d.pt' % i), env_index)
                if i % 10 == 0:
                ...
                if best_valid_loss is None or best_valid_loss - logger['state_loss_valid'] > 0.001:
                    best_valid_loss = logger['state_loss_valid']
                    epochs_since_update = 0
                    self.save_env_state_part(os.path.join(save_dir, 'model_state_best.pt'), env_index)
                    best_epoch = i
                else:
    ...
    def env_reward_train(self, batch_size, epoch, save_dir, valid_size=5000, max_epochs_since_update=5, env_index=0,
                         random_action_num=1):
    ...
    def env_train(self, batch_size, epoch, save_dir, valid_size=5000, max_epochs_since_update=5, env_index=0,
        ...
        if train_state:
            self.env_state_train(batch_size, epoch, os.path.join(save_dir, state_path), valid_size,
                                 max_epochs_since_update, env_index)
        if train_reward:
            self.env_reward_train(batch_size, epoch, os.path.join(save_dir, reward_path), valid_size,
                                  max_epochs_since_update, env_index, random_action_num)
        return

    def roll_out(self, init_state_batch, length=1):
        with torch.no_grad():
            state = init_state_batch
            for _ in range(length):
                action = self.actor.get_action(state, dtype='Tensor')
                next_state_mean_ensemble, next_state_std_ensemble, reward_ensemble = self.env_ensemble_function(
                    self.env_ensemble_params,
                    self.env_ensemble_buffers, state, action)
                batch_size = next_state_mean_ensemble.shape[1]
                model_index = np.random.choice(len(self.best_env_state_model_index), batch_size)

                next_state_mean = next_state_mean_ensemble[model_index, np.arange(0, batch_size)]
                next_state_std = next_state_std_ensemble[model_index, np.arange(0, batch_size)]
                noise = torch.randn_like(next_state_std)
                next_state = next_state_mean + noise * next_state_std
                reward = reward_ensemble.mean(dim=0)
                state_np = state.detach().cpu().numpy()
                action_np = action.detach().cpu().numpy()
                reward_np = reward.detach().cpu().numpy()[:, 0]
                next_state_np = next_state.detach().cpu().numpy()
                done_np = self.termination_fn(state_np, action_np, next_state_np)
                for i in range(state.shape[0]):
                    self.model_buffer.push(state_np[i], action_np[i], reward_np[i]+15*state_np[i, 0], next_state_np[i], done_np[i])
                state = next_state[~done_np]
                if state.shape[0] == 0:
                    break
        return

    def save_env_part(self, save_path, env_index=0):
        torch.save({'envModel': self.env_models[env_index].state_dict(),
                    'normalizer': self.env_models[env_index].normalizer}, save_path)

    def save_env_state_part(self, save_path, env_index=0):
        torch.save({'envStateModel': self.env_models[env_index].state_predictor.state_dict(),
                    'max_logstd': self.env_models[env_index].max_logstd,
                    'min_logstd': self.env_models[env_index].min_logstd,
                    'normalizer': self.env_models[env_index].normalizer}, save_path)
    ...
    def load_env_state_part(self, save_path, env_index=0):
        payload = torch.load(save_path)
        self.env_models[env_index].state_predictor.load_state_dict(payload['envStateModel'])
        self.env_models[env_index].max_logstd = payload['max_logstd']
        self.env_models[env_index].min_logstd = payload['min_logstd']
        self.env_models[env_index].normalizer = payload['normalizer']
        return
    ...
    def load_env_part(self, save_path, env_index=0):
        payload = torch.load(save_path)
        self.env_models[env_index].load_state_dict(payload['envModel'])
        self.env_models[env_index].normalizer = payload['normalizer']
        return

    def load_best_env_state_parts(self, env_path, state_path, best_env_nums):
        self.best_env_state_model_index = []
        model_metric = []
        for i in range(self.env_nums):
            with open(os.path.join(env_path + f'-{i}', state_path, 'model_state_log.csv'), 'r') as f:
                lines = f.readlines()
                score = float(lines[-1].split(',')[1][11:-1])
                model_metric.append((score, i))
        model_metric.sort()
        for i in range(best_env_nums):
            self.best_env_state_model_index.append(model_metric[i][1])
        for i in range(len(self.best_env_state_model_index)):
            self.load_env_state_part(
                os.path.join(env_path + f'-{self.best_env_state_model_index[i]}', state_path, 'model_state_best.pt'),
                env_index=i)

    def load_best_env_reward_parts(self, env_path, reward_path, best_env_nums):
        self.best_env_reward_model_index = []
        model_metric = []
        for i in range(self.env_nums):
            with open(os.path.join(env_path + f'-{i}', reward_path, 'model_reward_log.csv'), 'r') as f:
                lines = f.readlines()
                score = float(lines[-1].split(',')[1][11:-1])
                model_metric.append((score, i))
        model_metric.sort()
        for i in range(best_env_nums):
            self.best_env_reward_model_index.append(model_metric[i][1])
        for i in range(len(self.best_env_reward_model_index)):
            self.load_env_reward_part(
                os.path.join(env_path + f'-{self.best_env_reward_model_index[i]}', reward_path, 'model_reward_best.pt'),
                env_index=i)

    def load_best_env_models(self, env_path, state_path, reward_path, best_env_nums):
        self.load_best_env_state_parts(env_path, state_path, best_env_nums)
        self.load_best_env_reward_parts(env_path, reward_path, best_env_nums)
        fmodel, self.env_ensemble_params, self.env_ensemble_buffers = combine_state_for_ensemble(
            [self.env_models[i] for i in range(len(self.best_env_state_model_index))])
        self.env_ensemble_function = vmap(fmodel, in_dims=(0, 0, None, None))
    ...
    def recompute_reward(self, batch_size=64):
        ...
        for batch_num in range(int(np.ceil(len(self.offline_buffer) / batch_size))):
            state = torch.FloatTensor(self.offline_buffer.observations[batch_num * batch_size:min((batch_num + 1) * batch_size, data_size)]).to(device)
            action = torch.FloatTensor(self.offline_buffer.actions[batch_num * batch_size:min((batch_num + 1) * batch_size, data_size)]).to(device)
            reward = torch.FloatTensor(self.offline_buffer.rewards[batch_num * batch_size:min((batch_num + 1) * batch_size, data_size)]).unsqueeze(1).to(
                device)  # shape=(batch_size, 1)
            _, _, reward_ensemble = self.env_ensemble_function(
                self.env_ensemble_params,
                self.env_ensemble_buffers, state, action)
            self.offline_buffer.rewards[batch_num * batch_size:min((batch_num + 1) * batch_size, data_size)] = reward_ensemble.mean(dim=0).detach().cpu().numpy()[:, 0] \
                                                                                                               + 15 * state.detach().cpu().numpy()[:, 0]

    def RL_train(self, offline_batch_size, model_batch_size, train_step, save_dir, roll_out_length=1,
                 critic_update_pre_step=1, BC_initial_step=0):
        ...
        for i in range(int(train_step) + 1):
            # if i == 24500:
            #     print(1)
            init_states, _, _, _, _ = self.offline_buffer.sample(20)
    ...
    def RL_test(self, test_num=10):
        ...
        for _ in range(test_num):
            ...
            while not done:
                ...
                reward += state.detach().cpu().numpy()[0]
...
if __name__ == '__main__':
    ...
    #                     default='/data/lhdata/model-based-offline/C12/hopper-m-r/')
    ...
    # parser.add_argument("--state-path", type=str, help="the dir to save state part of environment", default='state')
    ...
    # parser.add_argument("--alpha-lr", type=float, help="learning rate of alpha", default=3e-5)
    ...
    # parser.add_argument('--model-replay-buffer-capacity', type=int, default=10000)
    ...
    trainer = CROPTrainer(state_dim=np.prod(d4rl_env.observation_space.shape),
                          ...
                          model_replay_buffer_capacity=args.model_replay_buffer_capacity,
    ...
    if args.train_state or args.train_reward:
        for i in range(args.env_nums):
            print(f'Start train environment model {i}')
            if args.train_state:
                if not os.path.exists(os.path.join(args.save_path, args.env_path + f'-{i}', args.state_path)):
                    os.makedirs(os.path.join(args.save_path, args.env_path + f'-{i}', args.state_path))
                with open(os.path.join(args.save_path, args.env_path + f'-{i}', args.state_path, 'args.json'),
                          'w') as f:
                    json.dump(vars(args), f, sort_keys=True, indent=4)

                trainer.env_state_train(args.model_batch_size, args.model_epoch,
                                        os.path.join(args.save_path, args.env_path + f'-{i}', args.state_path),
                                        args.valid_size,
                                        args.max_epochs_since_update, i)
            if args.train_reward:
                if not os.path.exists(os.path.join(args.save_path, args.env_path + f'-{i}', args.reward_path)):
                    os.makedirs(os.path.join(args.save_path, args.env_path + f'-{i}', args.reward_path))
                with open(os.path.join(args.save_path, args.env_path + f'-{i}', args.reward_path, 'args.json'),
                          'w') as f:
                    json.dump(vars(args), f, sort_keys=True, indent=4)

                trainer.env_reward_train(args.model_batch_size, args.model_epoch,
                                         os.path.join(args.save_path, args.env_path + f'-{i}', args.reward_path),
                                         args.valid_size,
